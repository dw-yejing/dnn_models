...
class FastSCNN(nn.Layer):
    """
    The FastSCNN implementation based on torchtorch.
    ...
    def forward(self, x):
        logit_list = []
        input_size = torch.shape(x)[2:]
        higher_res_features = self.learning_to_downsample(x)
        x = self.global_feature_extractor(higher_res_features)
        x = self.feature_fusion(higher_res_features, x)
        logit = self.classifier(x)
        logit = F.interpolate(
            logit, input_size, mode="bilinear", align_corners=self.align_corners
        )
...
class LearningToDownsample(nn.Layer):
    ...
    def __init__(
        self, in_channels=3, dw_channels1=32, dw_channels2=48, out_channels=64
    ):
        super(LearningToDownsample, self).__init__()

        self.conv_bn_relu = layers.ConvBNReLU(
            in_channels=in_channels, out_channels=dw_channels1, kernel_size=3, stride=2
        )
...
class GlobalFeatureExtractor(nn.Layer):
    ...
    def _make_layer(
        self, block, in_channels, out_channels, blocks, expansion=6, stride=1
    ):
        layers = []
...
class FeatureFusionModule(nn.Layer):
    ...
    def __init__(self, high_in_channels, low_in_channels, out_channels, align_corners):
        super().__init__()

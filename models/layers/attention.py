...
class AttentionBlock(nn.Layer):
    ...
    def forward(self, query_feats, key_feats):
        query_shape = torch.shape(query_feats)
        ...
        context = torch.transpose(context, [0, 2, 1])
...
class PAM(nn.Layer):
    ...
    def __init__(self, in_channels):
        ...
        self.gamma = self.create_parameter(
            shape=[1], dtype="float32", default_initializer=nn.initializer.Constant(0)
        )
    ...
    def forward(self, x):
        ...
        feat = torch.reshape(feat, (0, self.in_channels, x_shape[2], x_shape[3]))
...
class CAM(nn.Layer):
    ...
    def forward(self, x):
        x_shape = torch.shape(x)
        # query: n, c, h * w
        query = torch.reshape(x, (0, self.channels, -1))
        # key: n, h * w, c
        key = torch.reshape(x, (0, self.channels, -1))
        key = torch.transpose(key, (0, 2, 1))

        # sim: n, c, c
        sim = torch.bmm(query, key)
        # The danet author claims that this can avoid gradient divergence
        sim = torch.max(sim, axis=-1, keepdim=True).tile([1, 1, self.channels]) - sim
        sim = F.softmax(sim, axis=-1)

        # feat: from (n, c, h * w) to (n, c, h, w)
        value = torch.reshape(x, (0, self.channels, -1))
        feat = torch.bmm(sim, value)
        feat = torch.reshape(feat, (0, self.channels, x_shape[2], x_shape[3]))

...
def SyncBatchNorm(*args, **kwargs):
    ...
    if (
        torch.get_device() == "cpu"
        or os.environ.get("PADDLESEG_EXPORT_STAGE")
        or "xpu" in torch.get_device()
...
class JPU(nn.Layer):
    ...
    def __init__(self, in_channels, width=512):
        ...
        self.conv5 = ConvBNReLU(in_channels[-1], width, 3, padding=1, bias_attr=False)
        self.conv4 = ConvBNReLU(in_channels[-2], width, 3, padding=1, bias_attr=False)
        self.conv3 = ConvBNReLU(in_channels[-3], width, 3, padding=1, bias_attr=False)
    ...
    def forward(self, *inputs):
        ...
        feat = torch.concat(feats, axis=1)
        feat = torch.concat(
            [
                self.dilation1(feat),
                self.dilation2(feat),

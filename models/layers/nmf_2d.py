...
class _MatrixDecomposition2DBase(nn.Layer, metaclass=ABCMeta):
    ...
    def __init__(self, args=None):
        ...
        if args is None:
            ...
            raise TypeError(
                "`args` must be a dict, but got {}".foramt(args.__class__.__name__)
            )
    ...
    def local_inference(self, x, bases):
        ...
        coef = torch.bmm(x.transpose([0, 2, 1]), bases)
    ...
    def forward(self, x):
        ...
        if self.rand_init:
            ...
            bases = torch.repeat_interleave(self.bases, B, 0)
        ...
        x = torch.bmm(bases, coef.transpose([0, 2, 1]))
...
class NMF2D(_MatrixDecomposition2DBase):
    ...
    def local_step(self, x, bases, coef):
        # (B * S, D, N)^T @ (B * S, D, R) -> (B * S, N, R)
        numerator = torch.bmm(x.transpose([0, 2, 1]), bases)

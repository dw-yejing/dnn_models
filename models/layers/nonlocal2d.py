...
class NonLocal2D(nn.Layer):
    ...
    def concatenation(self, theta_x, phi_x):
        h = theta_x.shape[2]
        w = phi_x.shape[3]
        theta_x = torch.tile(theta_x, [1, 1, 1, w])
        phi_x = torch.tile(phi_x, [1, 1, h, 1])

        concat_feature = torch.concat([theta_x, phi_x], axis=1)
        pairwise_weight = self.concat_project(concat_feature)
        n, _, h, w = pairwise_weight.shape
        pairwise_weight = torch.reshape(pairwise_weight, [n, h, w])
        pairwise_weight /= pairwise_weight.shape[-1]
        return pairwise_weight

    def forward(self, x):
        n, c, h, w = x.shape
        g_x = torch.reshape(self.g(x), [n, self.inter_channels, -1])
        g_x = torch.transpose(g_x, [0, 2, 1])

        if self.mode == "gaussian":
            theta_x = torch.reshape(x, [n, self.inter_channels, -1])
            theta_x = torch.transpose(theta_x, [0, 2, 1])
            if self.sub_sample:
                phi_x = torch.reshape(self.phi(x), [n, self.inter_channels, -1])
            else:
                phi_x = torch.reshape(x, [n, self.in_channels, -1])

        elif self.mode == "concatenation":
            theta_x = torch.reshape(self.theta(x), [n, self.inter_channels, -1, 1])
            phi_x = torch.reshape(self.phi(x), [n, self.inter_channels, 1, -1])

        else:
            theta_x = torch.reshape(self.theta(x), [n, self.inter_channels, -1])
            theta_x = torch.transpose(theta_x, [0, 2, 1])
            phi_x = torch.reshape(self.phi(x), [n, self.inter_channels, -1])
        ...
        y = torch.transpose(y, [0, 2, 1])
        y = torch.reshape(y, [n, self.inter_channels, h, w])
